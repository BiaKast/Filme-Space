...
const visualArtifacts: Array<keyof MovieImages> = ["backdrops", "posters", "logos"];
...
export default function Pictures({ images }: { images: MovieImages }) {
  ...
  const selectedImages = images && visualArtifacts[statusVisualArtifacts] in images
  ...
  return (
    <div className="mt-6 flex flex-col gap-6">
      <div className="w-full text-center">
        <h2 className="text-3xl font-bold text-purple-500 uppercase">Artefatos Visuais</h2>
        <p className="text-gray-300 mt-2">Explore os cenários, cartazes e logotipos deste filme.</p>
      </div>
          ...
          <div className="flex w-full justify-around text-white font-semibold">
            <button
              className={`flex items-center justify-center w-32 py-2 rounded-lg transition ${
                statusVisualArtifacts === 0 ? "text-cyan-400 font-bold" : "text-white"
              }`} 
              id="0"
            ...
            <button
              className={`flex items-center justify-center w-32 py-2 rounded-lg transition ${
                statusVisualArtifacts === 1 ? "text-cyan-400 font-bold" : "text-white"
              }`} 
              id="1"
              onClick={toggleSortName}
            >
            ...
            <button
              className={`flex items-center justify-center w-32 py-2 rounded-lg transition ${
                statusVisualArtifacts === 2 ? "text-cyan-400 font-bold" : "text-white"
              }`} 
              id="2"
              onClick={toggleSortName}
            >
        ...
        <div className="relative w-full max-w-4xl mx-auto overflow-hidden">
          <div className="flex transition-transform duration-500 ease-in-out" style={{ transform: `translateX(-${currentIndex * 100}%)` }}>
            {selectedImages.length > 0 ? (
              selectedImages.map((image: { file_path: string }, index: number) => (
              ...
              <div className="w-full text-center py-10 flex flex-col items-center gap-2">
                ...
                <p className="text-gray-400">Nenhuma imagem disponível nesta categoria.</p>
              </div>
            )}
          </div>
          ...
          {selectedImages.length > 1 && !modalOpen && (
            <>
              <button
                onClick={() => setCurrentIndex((prev) => (prev === 0 ? selectedImages.length - 1 : prev - 1))}
              ...
              <button
                onClick={() => setCurrentIndex((prev) => (prev === selectedImages.length - 1 ? 0 : prev + 1))}
