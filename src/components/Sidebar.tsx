...
export default function Sidebar() {
  ...
  const handleSortChange = ({ nameSort, dateSort }: { nameSort: string; dateSort: string }) => {
  ...
  }, [sortNameState, sortDateState, selectYear, searchQuery, selectedCategory, selectedGenre, years, page, originalArray]);
  ...
  const handleCategoryChange = (category: string) => {
    ...
    setSelectedCategory({category, name: categories.find((c) => c.key === category)?.name || ""})
    setPage(1)
      ...
      >
        ...
        <div className="space-y-4">
          <div className="space-y-2">
            <label className="block w-full text-left hover:bg-gray-700 p-2 rounded transition">
              ...
              >
                ...
                >
                  <p className="text-xs">{sortNameState === 0 ? "Z-A" : sortNameState === 1 ? "Original" : "A-Z"}</p>
                </div>
              </button>
            </label>
          </div>
           ...
           <div className="space-y-2"> 
            <label className="block w-full text-left hover:bg-gray-700 p-2 rounded transition">
              ...
              >
                ...
                >
                  <p className="text-xs">{sortDateState === 0 ? "Antigo" : sortDateState === 1 ? "Nenhum" : "Recente"}</p>
                </div>
              </button>
            </label>
          </div>
          ...
          <div className="flex flex-col gap-2">
            <Label htmlFor="decades" className="text-sm">Décadas:</Label>
            <select
              id="decades"
          ...
          <div className="flex flex-col gap-2">
            <Label htmlFor="categories" className="text-sm">Categorias:</Label>
            <select
              id="categories"
          ...
          <div className="flex flex-col gap-2">
            <Label htmlFor="genres" className="text-sm">Gêneros:</Label>
            <select
              id="genres"
              value={selectedGenre}
